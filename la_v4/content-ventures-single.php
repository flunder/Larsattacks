<li class="module-ventures-item v-centre-desktop la-gallery module-ventures-leftie">

    <div class="col-sm-4 module-ventures-info bg-white v-centre-desktop-inner la-gallery-container-nav">

        <h3 class="h5">Bikeshd London</h3>

        <p>
            originally came to fruition to keep owners of stolen bikes in
            the loop about bikes being shifted through Gumtree and Ebay.
            Great idea, great purpose. But it serves a great double
            application—it also serves as a nifty filter when you.
        </p>

        <a href="#" class="font-bold">www.bikeshd.co.uk</a>

    </div>

    <div class="col-sm-8 module-ventures-gallery paddingless bg-dark v-centre-desktop-inner la-gallery-container-pagination">
        <figure class="cover bgless">
            <img src="<?php echo get_template_directory_uri() ?>/images/projects/bikeshd/1.jpg" alt="" />
            <img src="<?php echo get_template_directory_uri() ?>/images/projects/bikeshd/2.jpg" alt="" />
            <img src="<?php echo get_template_directory_uri() ?>/images/projects/bikeshd/3.jpg" alt="" />
        </figure>
    </div>

</li>

<li class="module-ventures-li v-center la-gallery module-ventures-rightie">

    <div class="col-sm-8 module-ventures-gallery paddingless bg-dark v-centre-desktop-inner la-gallery-container-pagination">
        <figure class="cover bgless">
            <img src="<?php echo get_template_directory_uri() ?>/images/projects/bikeshd/1.jpg" alt="" />
            <img src="<?php echo get_template_directory_uri() ?>/images/projects/bikeshd/2.jpg" alt="" />
            <img src="<?php echo get_template_directory_uri() ?>/images/projects/bikeshd/3.jpg" alt="" />
        </figure>
    </div>

    <div class="col-sm-4 module-ventures-info bg-white v-centre-desktop-inner la-gallery-container-nav">

        <h3 class="h5">Rackjam</h3>

        <p>
            originally came to fruition to keep owners of stolen bikes in
            the loop about bikes being shifted through Gumtree and Ebay.
            Great idea, great purpose. But it serves a great double
            application—it also serves as a nifty filter when you.
        </p>

        <a href="#" class="font-bold">www.bikeshd.co.uk</a>

    </div>

</li>